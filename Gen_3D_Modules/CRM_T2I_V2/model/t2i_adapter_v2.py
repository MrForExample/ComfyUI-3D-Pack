...
class ImageConv2dEncoder(nn.Module):
    """
    Quoting from https://arxiv.org/abs/2302.05543: "Stable Diffusion uses a pre-processing method similar to VQ-GAN
    [11] to convert the entire dataset of 512 × 512 images into smaller 32 × 32 “latent images” for stabilized
    training. We use a tiny network E(·) of four convolution layers with 4 × 4 kernels and 2 × 2 strides
    ...
    def __init__(
        ...
        output_embedding_channels: int = 320
    ):
        super().__init__()
...
class MultiAdapterV2(ModelMixin):
    r"""
    ...
    Parameters:
        adapters (`List[T2IAdapterV2]`, *optional*, defaults to None):
            A list of `T2IAdapterV2` model instances.
    """

    def __init__(self, adapters: List["T2IAdapterV2"]):
        super(MultiAdapterV2, self).__init__()
    ...
    def from_pretrained(cls, pretrained_model_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        while os.path.isdir(model_path_to_load):
            adapter = T2IAdapterV2.from_pretrained(model_path_to_load, **kwargs)
...
class T2IAdapterV2(ModelMixin, ConfigMixin):
    r"""
    ...
    def __init__(
        ...
        conditioning_embedding_out_channels: Optional[Tuple[int, ...]] = (16, 32, 64, 128, 256),
        adapter_type: str = "full_adapter",
    ):
        super().__init__()

        if adapter_type == "full_adapter":
            self.adapter = FullAdapterV2(in_channels, channels, num_res_blocks, conditioning_embedding_out_channels)
        elif adapter_type == "full_adapter_xl":
            self.adapter = FullAdapterXLV2(in_channels, channels, num_res_blocks, conditioning_embedding_out_channels)
        elif adapter_type == "light_adapter":
            self.adapter = LightAdapterV2(in_channels, channels, num_res_blocks, conditioning_embedding_out_channels)
        else:
            raise ValueError(
    ...
    def downscale_factor(self):
        return self.adapter.downscale_factor


# full adapter


class FullAdapterV2(nn.Module):
    r"""
    ...
    def __init__(
        ...
        img_embedding_channel = channels[0]
        
        self.conv2d_img_encoder = ImageConv2dEncoder(
            conditioning_channels=in_channels,
            block_out_channels=conditioning_embedding_out_channels,
            output_embedding_channels=img_embedding_channel,
        )

        self.body = nn.ModuleList(
            [
                AdapterBlock(img_embedding_channel, img_embedding_channel, num_res_blocks),
                *[
        ...
        self.total_downscale_factor = self.downscale_factor * 2 ** (len(channels) - 1)

    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        r"""
        This method processes the input tensor `x` through the FullAdapter model and performs operations including
        stack of convolutions, and a stack of AdapterBlocks. It returns a list of feature tensors, each
...
class FullAdapterXLV2(nn.Module):
    r"""
    See [`T2IAdapter`] for more information.
    """
    ...
    def __init__(
        ...
        self.downscale_factor = 16
        self.total_downscale_factor = self.downscale_factor * 2

    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        r"""
        This method takes the tensor x as input and processes it through FullAdapterXL model. It consists of operations
        stack of convolution layers and appending each block into list of feature tensors.
        """
        x = self.conv2d_img_encoder(x)
...
class LightAdapterV2(nn.Module):
    r"""
    See [`T2IAdapter`] for more information.
    """
    ...
    def __init__(
        ...
        self.body = nn.ModuleList(
            [
                LightAdapterBlock(img_embedding_channel, img_embedding_channel, num_res_blocks),
                *[
        ...
        self.downscale_factor = 16
        self.total_downscale_factor = self.downscale_factor * (2 ** len(channels))
