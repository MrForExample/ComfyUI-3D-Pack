        ...
        **kwargs,
        ...
        for i, step in enumerate(iterator):
            ...
            if index % log_every_t == 0 or index == total_steps - 1:
                intermediates["x_inter"].extend(img)
    ...
    def p_sample_ddim(
        ...
        **kwargs,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            raw_model_output = self.model.apply_model(x_in, t_in, c_in, **kwargs)
            if isinstance(raw_model_output, torch.Tensor):
                raw_model_output = [raw_model_output]

            all_x_prev = []
            all_pred_x0 = []
            all_e_t = []
            for raw_output, x_input in zip(raw_model_output, x.chunk(3, dim=1)):
                model_uncond, model_t = raw_output.chunk(2)
                ...
                if self.model.parameterization == "v":
                    print("using v!")
                    e_t = self.model.predict_eps_from_z_and_v(x_input, t, model_output)
                else:
                    e_t = model_output
                ...
                if score_corrector is not None:
                    ...
                    e_t = score_corrector.modify_score(
                        self.model, e_t, x_input, t, c, **corrector_kwargs
                    )
                ...
                if self.model.parameterization != "v":
                    pred_x0 = (x_input - sqrt_one_minus_at * e_t) / a_t.sqrt()
                else:
                    pred_x0 = self.model.predict_start_from_z_and_v(x_input, t, model_output)
                ...
                noise = sigma_t * noise_like(x_input.shape, device, repeat_noise) * temperature
        ...
        return all_x_prev, all_pred_x0, all_e_t

    def add_noise(self, x0, t, noise=None, use_original_steps=False):
        ...
        # sqrt_alphas_cumprod: torch.Size([50]); t torch.Size([1]); x0: torch.Size([6, 4, 32, 32])
        return (
