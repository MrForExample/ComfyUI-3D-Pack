...
class ConvolutionalReconstructionModel(nn.Module):
    ...
    @torch.no_grad()
    def get_mesh_wt_uv(self, ctx, data, device, res, tri_fea_2=None):
        ...
        mesh_v = torch.tensor(verts, dtype=torch.float32, device=device)
        mesh_f = torch.tensor(faces, dtype=torch.int32, device=device)
        mesh_vt = uvs.clone().detach()
        mesh_ft = torch.tensor(indices.astype(np.int32), dtype=torch.int32, device=device)
        albedo = torch.tensor(img, dtype=torch.float32, device=device)
        
        mesh = Mesh(v=mesh_v, f=mesh_f, vt=mesh_vt, ft=mesh_ft, albedo=albedo, device=device)
        mesh.auto_normal()
        return mesh