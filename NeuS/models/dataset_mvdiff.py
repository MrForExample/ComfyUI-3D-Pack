...
def load_pose_and_preprocess(mv_images, mv_masks, mv_normals, imSize, view_types, cam_pose_dir=None, normal_system='front'):
    ...
    img_H, img_W = imSize[0], imSize[1]
    
    # rgb to bgr
    mv_images = mv_images[:, :, :, [2, 1, 0]].contiguous()
    
    for idx, view in enumerate(view_types):
        
        image = prepare_torch_img(mv_images[idx].unsqueeze(0), img_H, img_W, keep_shape=True).squeeze(0).detach().cpu().numpy()
        mask = prepare_torch_img(mv_masks[idx].unsqueeze(2).unsqueeze(0), img_H, img_W, keep_shape=True).squeeze(0).squeeze(2).detach().cpu().numpy()
        normal = prepare_torch_img(mv_normals[idx].unsqueeze(0), img_H, img_W, keep_shape=True).detach().squeeze(0).cpu().numpy()
...
class Dataset:
    def __init__(self, mv_images, mv_masks, mv_normals, conf, cam_pose_dir, num_views=6):
        ...
        self.images_np, self.masks_np, self.normals_cam_np, self.normals_world_np ,self.pose_all_np, self.w2c_all_np = load_pose_and_preprocess(
            mv_images, mv_masks, mv_normals, self.imSize, view_types, cam_pose_dir
        )

        self.n_images = num_views
        ...
        self.intrinsics_all = torch.stack([self.intrinsic]*num_views, dim=0).cpu()
    ...
    def get_batch_of_random_rays(self, batch_size):
        """
        Get batch of random rays at world space.
        """
        batch_index = torch.randint(low=0, high=self.__len__(), size=[batch_size])
        return self.all_rays[batch_index, :]    # batch_size, 14
